...
// Dealer: Role(1/4) → Phone(2/4) → Details(3/4) → PIN(4/4) → Pending
...
export default function DealerSignup() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      navigate('/verify-otp', {
        state: { phone, role: 'dealer', isSignUp: true, nextRoute: '/signup/dealer/details' },
        replace: true,
      })
    } catch (e) {
      const raw = e.message || ''
  ...
  return (
    <div className="page-shell grain dealer-theme">
      ...
      <div className="page-body">
          ...
          {[0,1,2].map(i => (
            <div key={i} className="h-1.5 rounded-full transition-all"
              style={{
                ...
                background: i <= 1 ? '#EF9F27' : 'var(--card-br)',
              }} />
          ))}
        </div>
        ...
        <div className="anim-1">
          <div className="field-wrap">
            ...
            <p className="field-hint">A 6-digit verification code will be sent to this number</p>
          </div>

          {/* Progress overview */}
          <StepProgress currentStep={2} role="dealer" />
      ...
      <div className="page-cta">
        <button className="btn-main amber" onClick={handleSend} disabled={loading}>
          {loading ? <><span className="spinner" /> Sending code…</> : 'Send verification code →'}
        </button>
        <p className="cta-note" style={{ "--cta-link-color": "rgba(239,159,39,0.8)" }}>
