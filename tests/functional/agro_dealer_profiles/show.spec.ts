    ...
    .run(async ({ client, route }, condition) => {
      ...
      if (condition !== 'not_logged_in') {
        tokenValue = await generateLoginToken({
          user: condition === 'not_agro_dealer' ? farmer.user : targetDealer.user,
        })
      }
      ...
      response.assertBodyContains({
        data: {
          ...
          agroDealerProfile: {
            ...
            bank_name: targetDealer.bank_name,
            bank_account_number: targetDealer.bank_account_number,
            bank_account_name: targetDealer.bank_account_name,
